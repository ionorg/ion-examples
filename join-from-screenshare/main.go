...
// and publishing the local X screen
package main
...
func main() {
	...
	rootCmd.PersistentFlags().StringVarP(&username, "ion_username", "u", "join-from-screenshare", "username to join with")
	rootCmd.PersistentFlags().IntVarP(&width, "video_width", "w", 720, "video width to scale to")
	viper.AutomaticEnv()
	...
	width = viper.GetInt("video_width")
...
func doJoin() {
	...
	s, err := md.GetDisplayMedia(mediadevices.MediaStreamConstraints{
		Video: func(c *mediadevices.MediaTrackConstraints) {
			c.Enabled = true
			c.VideoTransform = video.Scale(-1, width, nil) // Resize to 360p
